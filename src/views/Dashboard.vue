  ...
  <div class="app">
    <main class="main">
      ...
      <div v-if="tab === 'dashboard'" class="tab-content fade-up">
        ...
        <div class="card breakdown-section" v-if="Object.keys(summary.by_category || {}).length">
          <h3>Category Breakdown</h3>
          <div class="breakdown-list">
            <div v-for="(amt, cat) in summary.by_category" :key="cat" class="bd-item">
              <div class="bd-left">
                <span class="bd-dot" :style="{ background: catColor(cat) }"></span>
                <span class="bd-name">{{ cat }}</span>
              </div>
              <div class="bd-right">
                <span class="bd-pct" v-if="summary.salary > 0">{{ Math.round((amt/summary.salary)*100) }}%</span>
                <span class="bd-amt">{{ fmt(amt) }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="tab === 'transactions'" class="tab-content fade-up">
        <div class="card add-form">
          ...
          <div class="form-grid">
            ...
            <input :value="displayAmount" type="text" placeholder="Amount" @input="onAmountInput" inputmode="numeric" />
...
.topbar { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-bottom: 1px solid var(--border); background: var(--bg2); position: sticky; top: 0; z-index: 10; }
...
.summary-card { position: relative; min-height: 90px; }
.s-label { font-size: 11px; color: var(--muted); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
.s-value { font-size: 18px; font-weight: 700; word-break: break-word; }
.green { color: var(--green); }
.red { color: var(--red); }
.edit-salary { position: absolute; top: 10px; right: 10px; background: none; border: 1px solid var(--border); color: var(--muted); padding: 3px 8px; font-size: 10px; border-radius: 6px; }
...
.chart-box { display: flex; justify-content: center; align-items: center; padding: 8px; }
.chart-box canvas { max-width: 240px !important; max-height: 240px !important; }
.empty-chart { text-align: center; color: var(--muted); padding: 32px 0; font-size: 13px; }

.breakdown-section h3 { font-size: 14px; margin-bottom: 12px; }
.breakdown-list { display: flex; flex-direction: column; gap: 8px; }
.bd-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid var(--border); }
...
.bd-name { font-size: 13px; color: var(--text); }
.bd-right { display: flex; align-items: center; gap: 10px; }
.bd-pct { font-size: 11px; color: var(--accent); background: rgba(201,169,110,0.12); padding: 3px 8px; border-radius: 12px; font-weight: 600; }
...
.suggestions { display: flex; flex-wrap: wrap; gap: 6px; justify-content: center; margin-top: 16px; }
.suggest-btn { padding: 8px 14px; border-radius: 20px; background: var(--bg3); border: 1px solid var(--border); color: var(--text); font-size: 12px; cursor: pointer; }
.suggest-btn:hover { border-color: var(--accent); color: var(--accent); }
.msg { display: flex; }
.msg.user { justify-content: flex-end; }
.msg.assistant { justify-content: flex-start; }
.bubble { max-width: 85%; padding: 10px 14px; border-radius: 14px; font-size: 13px; line-height: 1.5; white-space: pre-wrap; }
.msg.user .bubble { background: var(--accent); color: #0a0a0f; }
.msg.assistant .bubble { background: var(--bg3); border: 1px solid var(--border); }
...
.send-btn { width: auto; padding: 8px 16px; font-size: 18px; }

.bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; display: flex; background: var(--bg2); border-top: 1px solid var(--border); z-index: 50; }
.bottom-nav button { flex: 1; background: none; border: none; padding: 10px; color: var(--muted); display: flex; flex-direction: column; align-items: center; gap: 4px; cursor: pointer; transition: color 0.2s; }
